...
static unsigned int num_sessions = NUM_SESSIONS;
...
static int send_one_frame(snd_pcm_t *snd,
		...
		const unsigned int timestamp_per_frame,
		RtpSession *session[],
		SessionSet *set)
{
	int16_t *pcm;
	...
	for (int i=0; i<num_sessions; i++) {
		rtp_session_send_with_ts(session[i], packet, z,timestamp);
	}
	timestamp += timestamp_per_frame;
		...
		SessionSet *set)
{
	for (;;) {
		...
		result = send_one_frame(snd, channels, frame,
			encoder, bytes_per_frame, timestamp_per_frame,
			session, set);
		if (result == -1)
			return -1;
...
int main(int argc, char *argv[])
{
	...
	unsigned int timestamp_per_frame;
	snd_pcm_t *snd;
	OpusEncoder *encoder;
	...
	unsigned int range_start = IP_START;
	...
	encoder = opus_encoder_create(rate, channels, OPUS_APPLICATION_VOIP,
				&error);
	if (encoder == NULL) {
	...
	timestamp_per_frame = frame * 8000 / rate;

	ortp_init();
	ortp_scheduler_init();
	ortp_set_log_level_mask(NULL, ORTP_WARNING|ORTP_ERROR);

	RtpSession *session[sizeof(RtpSession) * num_sessions];
	SessionSet *set = session_set_new();

	char ip_addr[16];
	for (int i=0; i<num_sessions; i++) {	
		snprintf(ip_addr, sizeof(ip_addr), "%d.%d.%d.%d", 10, 3, 141, range_start + i);
		session[i] = create_rtp_send(ip_addr, port);
		assert(session[i] != NULL);
		session_set_set(set,session[i]);
	}

	result = snd_pcm_open(&snd, device, SND_PCM_STREAM_CAPTURE, 0);
	if (result < 0) {
		aerror("snd_pcm_open", result);
		return -1;
	}
	...
	go_realtime();
	result = run_tx(snd, channels, frame, encoder, bytes_per_frame,
		timestamp_per_frame, session, set);
